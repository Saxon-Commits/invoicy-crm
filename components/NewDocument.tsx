...
const TemplateCard: React.FC<{ to: string; title: string; description: string; icon: React.ReactNode }> = ({ to, title, description, icon }) => (
    <Link to={to} className="bg-white dark:bg-zinc-900 p-6 rounded-xl shadow-sm hover:shadow-lg hover:scale-105 transform transition-all duration-300 group">
...
const NewDocument: React.FC = () => {
    return (
        <div className="space-y-8 h-full overflow-y-auto p-4 sm:p-6 lg:p-8">
            <p className="text-lg text-slate-600 dark:text-zinc-400">Choose a template to get started. Each one is designed for a specific purpose.</p>
