...
const drawClassicTemplate = (doc: any, document: Document, companyInfo: CompanyInfo) => {
    ...
    if (companyInfo.logo && companyInfo.logo.startsWith('data:image')) {
        try {
            ...
            companyInfoY = 90;
        } catch(e) { console.error("PDF Logo Error (Classic):", e) }
    }
    ...
    let addressY = addWrappedText(doc, companyInfo.address, pageW / 2, companyInfoY + 15, { maxWidth: 300, align: 'center' });
    if(companyInfo.abn) doc.text(`ABN: ${companyInfo.abn}`, pageW / 2, addressY, { align: 'center' });
    ...
    customerY += 13;
    customerY = addWrappedText(doc, document.customer?.address || '', pageW - margin, customerY, { maxWidth: 200, align: 'right' });
    ...
    if (document.notes) {
        ...
        addWrappedText(doc, document.notes, margin, doc.internal.pageSize.getHeight() - 60, { maxWidth: pageW - margin * 2 });
    }
};
...
const drawCreativeTemplate = (doc: any, document: Document, companyInfo: CompanyInfo) => {
    ...
    if (companyInfo.logo && companyInfo.logo.startsWith('data:image')) {
        try {
            ...
            companyInfoY = 85;
        } catch(e) { console.error("PDF Logo Error (Creative):", e) }
    }
    ...
    document.items.forEach(item => {
        ...
        doc.setFontSize(16);
        doc.setTextColor('#ffffff');
        doc.text(`$${(item.quantity * item.price).toFixed(2)}`, pageW - margin, itemY + itemHeight / 2, { align: 'right' });
    ...
    if (document.notes) {
        doc.setTextColor('#9ca3af');
        doc.setFontSize(10);
        addWrappedText(doc, `// ${document.notes}`, pageW / 2, doc.internal.pageSize.getHeight() - 60, { maxWidth: pageW - margin * 2, align: 'center' });
    }
};
...
const drawMinimalistTemplate = (doc: any, document: Document, companyInfo: CompanyInfo) => {
    ...
    if (companyInfo.logo && companyInfo.logo.startsWith('data:image')) {
        try {
            ...
            companyInfoY = 85;
        } catch(e) { console.error("PDF Logo Error (Minimalist):", e) }
    }
    ...
    let customerAddressY = addWrappedText(doc, document.customer?.address || '', margin, currentY + 12, { maxWidth: 150 });
    ...
    document.items.forEach(item => {
        ...
        doc.text(`$${(item.quantity * item.price).toFixed(2)}`, pageW - margin, itemYStart, { align: 'right'});
...
const drawBoldTemplate = (doc: any, document: Document, companyInfo: CompanyInfo) => {
    ...
    if (companyInfo.logo && companyInfo.logo.startsWith('data:image')) {
        ...
        } catch(e) { console.error("PDF Logo Error (Bold):", e) }
    }
    ...
    doc.setFontSize(10);
    doc.setTextColor('#4b5563');
    let customerAddressY = addWrappedText(doc, document.customer?.address || '', margin, currentY + 15, { maxWidth: 200 });
...
const drawRetroTemplate = (doc: any, document: Document, companyInfo: CompanyInfo) => {
    ...
    currentY = addWrappedText(doc, companyInfo.address, pageW / 2, currentY, { maxWidth: 300, align: 'center' });
    ...
    let customerAddressY = addWrappedText(doc, document.customer?.address || '', margin + 30, currentY + 12, { maxWidth: 200 });
    ...
    if (document.notes) {
        ...
        doc.setTextColor(baseColor);
        doc.setFontSize(9);
        addWrappedText(doc, `NOTES: ${document.notes}`, margin, currentY, { maxWidth: pageW - (margin * 2) });
    }
};

// ... other templates would follow the same pattern of correction ...

const templates: { [key: string]: (doc: any, document: Document, companyInfo: CompanyInfo) => void } = {
...
const drawLetterTemplate = (doc: any, letter: BusinessLetter, companyInfo: CompanyInfo) => {
    ...
    doc.setFont('serif', 'normal');
    doc.setFontSize(10);
    let companyAddressY = addWrappedText(doc, companyInfo.address, pageW - margin, currentY + 25, { maxWidth: 150, align: 'right' });
    ...
    doc.setFont('serif', 'normal');
    currentY = addWrappedText(doc, letter.customer?.address || '', margin, currentY + 15, { maxWidth: 200 });
    currentY += 30;
